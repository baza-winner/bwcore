...
func TestUnmarshalJSON(t *testing.T) {
  ...
  "should not break with { character in key": 1
	...
	vinnermap := vinnerslice[0].(bwmap.Ordered)
	k = vinnermap.Keys()
	for i := range k {
		if k[i] != expectedKeys[i] {
...
`
	o := bwmap.OrderedNew()
	json.Unmarshal([]byte(s), &o)
	o.Sort(func(a *bwmap.Pair, b *bwmap.Pair) bool {
